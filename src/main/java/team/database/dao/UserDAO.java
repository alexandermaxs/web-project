...
public class UserDAO extends AbstractDAO {
    private static final String SQL_CREATE_USER = "INSERT INTO team.user (login, password, role) VALUES (?, ?, ?)";
    private static final String SQL_CHECK_IS_USER_EXIST = "SELECT login FROM team.user WHERE login = ?";
    private static final String SQL_READ_USER_BY_LOGIN_AND_PASS = "SELECT id, login, password, role, first_name FROM team.user WHERE login = ? and password = ?";
    ...
    public boolean isUserExist(String login) throws DAOException {
        ...
        try {
            this.connection = getConnection();
            PreparedStatement statement = connection.prepareStatement(SQL_CHECK_IS_USER_EXIST);
    ...
    public User getUserByLoginPassword(String login, String password) throws UnsupportedEncodingException, DAOException, InvalidArgumentException {
        ...
        try {
            this.connection = getConnection();
            PreparedStatement statement = connection.prepareStatement(SQL_READ_USER_BY_LOGIN_AND_PASS);
